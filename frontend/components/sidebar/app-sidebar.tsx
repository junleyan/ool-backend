...
import { Building, ChevronRightIcon, Tags, Users, X as CloseIcon, Sun, Moon, Clock, BrainCircuit, Loader2, Ban } from "lucide-react"
...
const AppSidebar = ({ state, dispatch }: { state: State, dispatch: Dispatch<{ type: string; payload: unknown }> }) => {
    ...
    const handleAISuggestionsClick = () => {
        dispatch({ type: "isLoadingAISuggestions", payload: true });
    }

    const handleExitAISuggestionsClick = () => {
        dispatch({ type: "displayAISuggestions", payload: false });
    }
            ...
            <SidebarContent className="flex">

                {
                    state.isLoadingAISuggestions ?
                        <Button variant={'outline'} className="mx-5 mt-3" disabled>
                            <Loader2 className="animate-spin" />
                            Please wait
                        </Button>
                        :
                        <Button variant={'outline'} className="mx-5 mt-3" onClick={handleAISuggestionsClick}>
                            <BrainCircuit /> AI Dataset Suggestions
                        </Button>
                }
                {
                    state.displayAISuggestions &&
                    <Button variant={'destructive'} className="mx-5 mt-1" onClick={handleExitAISuggestionsClick}>
                        <Ban /> Close Suggestion
                    </Button>
                }
                                    ...
                                    <SidebarMenuItem>
                                        ...
                                        <CollapsibleContent>
                                            <SidebarMenuSub>
                                                <Combobox
                                                    dispatch={dispatch}
                                                    disabled={state.isLoadingFilters || state.isLoadingAISuggestions}
